...
for i=tmax_c/dt+2:tmax_c/dt+tmax_s/dt+1
    for j =2:nx
        theta_f(i+1,j+1)=theta_f(i,j+1)+(dt/tmax)*((theta_f(i,j+1)-2*theta_f(i,j)+theta_f(i,j-1))/(peaf(i,j)*(dx/H)*(dx/H)));
        theta_s(i+1,j+1)=theta_s(i,j+1)+(dt/tmax)*((theta_s(i,j+1)-2*theta_s(i,j)+theta_s(i,j-1))/(peas*(dx/H)*(dx/H)) +K(i,j)*stfs(i,j)*(theta_f(i,j)-theta_s(i,j))-stw*(theta_s(i,j)-1));
    end
     theta_s(i,1)=theta_s(i-1,1)+stfs(i,j)*K(i,j)*(dt/tmax)*(theta_f(i,1)-theta_s(i-1,1));   %boundary conditions from table
     theta_s(i,nx+1)=theta_s(i-1,nx+1)+stfs(i,j)*K(i,j)*(dt/tmax)*(theta_f(i,nx+1)-theta_s(i-1,nx+1));
     theta_f(i,nx+1) = theta_f(i,nx);
     theta_f(i,1) = theta_f(i-1,1);
...
for i=tmax_c/dt+tmax_s/dt+2:nt
    for j = 2:nx
        theta_f(i+1,j+1)=theta_f(i,j+1)+(dt/tmax)*((theta_f(i,j+1)-2*theta_f(i,j)+theta_f(i,j-1))/(peaf(i,j)*(dx/H)*(dx/H))-stfs(i,j)*(theta_f(i,j)-theta_s(i,j))- (theta_f(i,j+1)-theta_f(i,j))/(dx/H));
        theta_s(i+1,j+1)=theta_s(i,j+1)+(dt/tmax)*((theta_s(i,j+1)-2*theta_s(i,j)+theta_s(i,j-1))/(peas*(dx/H)*(dx/H)) +K(i,j)*stfs(i,j)*(theta_f(i,j)-theta_s(i,j))-stw*(theta_s(i,j)-1));
    end
